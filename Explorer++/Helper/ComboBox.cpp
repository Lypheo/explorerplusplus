	...
	m_SuppressAutocomplete(false)
{
	...
	SetWindowSubclass(cbi.hwndItem, ComboBoxEditProcStub, m_SubclassCounter,
		reinterpret_cast<DWORD_PTR>(this));

	/* Subclass the parent window so that the CBN_EDITCHANGE can be caught. */
	SetWindowSubclass(GetParent(m_hwnd), ComboBoxParentProcStub, m_SubclassCounter,
		reinterpret_cast<DWORD_PTR>(this));
}
...
LRESULT CALLBACK ComboBox::ComboBoxEditProcStub(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam,
...
LRESULT CALLBACK ComboBox::ComboBoxParentProcStub(HWND hwnd, UINT uMsg, WPARAM wParam,
	...
	if (currentText.length() > 0)
	{
		...
		for (const auto &stringEntry : stringEntries)
		{
			if (stringEntry.compare(0, currentText.length(), currentText) == 0)
			{
				...
				the end of the text. */
				if (selectionStart == currentText.length() && selectionEnd == currentText.length())
				{
